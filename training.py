...
from torch.utils.tensorboard import SummaryWriter #run using this command -> tensorboard --logdir=runs
...
class SkinCancerClsTrainingApp:
    ...
    def initTrainDl(self):
        train_ds=SkinCancerDataset(is_test=False,is_val=False,val_stride=10)
    ...
    def main(self):
        ...
        for epoch_ndx in range(1, self.cli_args.epochs + 1):
            log.info("Epoch {} of {}, no of training batches {} / no of validation batches {} batches of size {}*{}".format(
                epoch_ndx,
            ...
            if(epoch_ndx !=0 and epoch_ndx % 5==0):
    ...
    def computeBatchLoss(self, batch_ndx, batch_tup, batch_size, metrics_g):
        ...
        loss_g = loss_func(
            logits_g,  # CrossEntropyLoss in PyTorch already applies softmax internally
            label_g[:,1], # the positive column
        )
    ...
    def saveModel(self, type_str, epoch_ndx, isBest=False):
        ...
        best_path = os.path.join(
            'saved_models',
            f'{type_str}_best.state'
        )

        os.makedirs(os.path.dirname(best_path), mode=0o755, exist_ok=True)
        ...
        log.info("Saved best model params to {}".format(best_path))
